...
const WithDrawTerms = () => {
  const { onClickPayment, withdrawOrder } = usePoint();
  ...
  const handleAddAmount = (amount: number) => {
    setChargeAmount((prev) => (prev ? Number(prev) + amount : amount));
  };
  ...
  const handlePaymentClick = () => {
    if (userInfo && Number(chargeAmount) > userInfo.point) {
      alert("보유 포인트보다 높은 금액을 요청하셨습니다.");
      return;
    }
  ...
  const handleAccountOwnerChange = (e: ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <div className={styles.ChargePointContainer}>
      <div className={styles.ChargePointTitle}>포인트 출금</div>
      <div className={styles.subscribeInfo}>
        <div className={styles.subTitle}>구독 정보</div>
        <div className={styles.subInfoWrapper}>
          <div className={styles.subInfoContainer}>
            <div className={styles.subInfoTitle}>보유 포인트</div>
            <div className={styles.subInfoData}>
              {userInfo
                ? userInfo.point
                  ? `${userInfo.point} P`
                  : "0 P"
          ...
          <div className={styles.subInfoContainer}>
            <div className={styles.subInfoTitle}>구독 만료일</div>
            <div className={styles.subInfoData}>
      ...
      <div className={styles.subscribeInfo2}>
        <div className={styles.subTitle}>전자금융거래약관</div>
        <div className={styles.subInfoWrapper}>
          <div className={styles.rightBox}>
            <div className={styles.rightTerm}>
              <pre>{`전자금융거래약관
제 1 장 총칙
제 1 조 (목적)
본 약관은 VAR(이하 "회사"라 합니다)가 제공하는 직불전자지급수단의 발행 및 관리 서비스, 선불전자지급수단의 발행 및 관리서비스, 전자지급결제대행서비스, 결제대금예치서비스 및 전자고지결제서비스(이하 통칭하여 "전자금융거래서비스"라 합니다)를 "회원"이 이용함에 있어, "회사"와 "회원" 간 권리, 의무 및 "회원"의 서비스 이용절차 등에 관한 사항을 규정하는 것을 그 목적으로 합니다.
제 2 조 (용어의 정의)
...
③ "회사"가 본 약관을 변경하는 때에는 그 시행일 1월 전에 변경되는 약관을 금융거래정보 입력화면 또는 서비스 홈페이지에 게시하고, "회원"에게 통지합니다. 다만, 법령의 개정으로 인하여 긴급하게 약관을 변경한 때에는 변경된 약관을 서비스 이용 초기화면 등에 1월 이상 게시하고 "회원"에게 사후 통지합니다.
...
VIP And Rendezvous, VAR
① 주소: 경기도 성남시 분당구 판교로 242 PDC A동 902호
② 이메일주소: dbgus1006@gmail.com
③ 전화번호: 000-0000-0000
제 5 조 (“거래지시”의 철회)
...
제 9 조 ("회사”의 책임)
① "회사"는 다음 각 호의 어느 하나에 해당하는 사고로 인하여 "회원"에게 손해가 발생한 경우에는 그 손해를 배상할 책임을 부담합니다.
...
4. “회사”가 “접근매체” 외에 보안강화를 위하여 전자금융거래 시 요구하는 추가적인 보안조치를 “회원”이 정당한 사유 없이 거부하여 본 약관 제9조제1항제3호에 따른 사고가 발생한 경우
5. “회원”이 제4호에 따른 추가적인 보안조치에 사용되는 매체ᆞ수단 또는 정보에 대하여 다음 각 목의 어느 하나에 해당하는 행위를 하여 법 제9조제1항제3호에 따른 사고가 발생한 경우
가. 누설ᆞ노출 또는 방치한 행위
...
제 22조 (정의) 본 장에서 사용하는 용어의 정의는 다음과 같습니다.
...
① 제 17조는 본 장에 준용합니다.
제 24 조 (환급 등)
① "회원"은 보유 중인 "선불전자지급수단"의 환급을 "회사"에 요구할 경우 환불수수료를 공제한 미상환 잔액에 대해 전액 환급받을 수 있습니다. 다만, 상품 구매, 이벤트 등을 통하여 "회사"로부터 무상 제공받은 "선불전자지급수단"의 경우 환급 대상에서 제외됩니다.
② “선불전자지급수단”의 환급 관련 환불수수료 부과여부 및 부과기준 관련 구체적인 내용은 홈페이지나 앱을 통해 사전에 공지합니다. 다만, 신규로 수수료를 부과하거나 기존 수수료를 변경하게 되는 경우 시행 1개월 전에 홈페이지나 앱을 통해 공지하고 “회원”에게 개별 통지합니다.
③ 제1항에도 불구하고, 다음 각 호의 어느 하나에 해당하는 경우에는 환불수수료 공제 없이 “선불전자지급수단”에 기록된 잔액 전부를 환급합니다.
1. 천재지변 등의 사유로 가맹점이 재화 또는 용역을 제공하기 곤란하여 “선불전자지급수단”을 사용하지 못하게 된 경우
2. “선불전자지급수단”의 결함으로 가맹점이 재화 또는 용역을 제공하지 못하는 경우
3. 유상 “선불전자지급수단”의 구매 금액(최종 유상 충전시 그 시점의 잔액 기준)의 100분의 60  (1만원 이하는 100분의 80 이상)을 사용한 경우
...
② 제16조는 본 장에 준용합니다.
제 27 조 (소멸시효 및 유효기간)
① "선불전자지급수단"의 소멸시효는 마지막 적립, 충전일 또는 사용일로부터 10년입니다.
② “회사”가 "선불전자지급수단”의 유효기간을 별도로 정하지 아니한 경우, 제1항에서 정한 소멸시효기간(10년)을 유효기간으로 봅니다.
③ “회사”는 “회원”이 유상으로 충전한 “선불전자지급수단”의 유효기간이 도래하기 30일전 통지를 포함하여 3회 이상 “회원”에게 유효기간의 도래, 유효기간의 연장 가능여부와 방법, 유효기간 경과 후 잔액의 90%를 반환받을 수 있다는 내용 등을 이메일 또는 알림 등의 방법으로 통지하여야 합니다.
④ 제3항에도 불구하고, “회사”가 무상으로 적립·제공한 “선불전자지급수단”의 경우, “회사”는 “무상 선불전자지급수단”의 유효기간이 도래하기 전 이메일, 알림 등을 통해 소멸예정 무상 “선불전자지급수단”에 대해 안내합니다.
제 28 조 (선불충전금의 관리 및 관련 공시)
① “회사”는 “회원”이 유상으로 충전한 “선불전자지급수단”의 선불충전금을 고유재산과 구분하여 외부 금융기관에 신탁하거나 지급보증보험에 가입하여야 합니다.
② “회사”는 매 영업일 마다 선불충전금 총액과 신탁금 등 실제 운용중인 자금 총액의 상호일치 여부를 점검하여야 하며, 매 분기말(분기 종료 후 10일 이내) 선불충전금 규모 및 신탁내역, 지급보증보험 가입여부, 부보 금액 등을 홈페이지에 공시하여야 합니다.
③ “회사”는 다음 각 호의 어느 하나에 해당하는 경우 선불충전금을 신탁회사 등을 통하여 “회원”에게 우선 지급하여야 합니다. 이 경우 1개월 이내에 그 사실과 선불충전금의 지급시기, 지급장소, 그 밖에 선불충전금의 지급과 관련된 사항을 둘 이상의 일간신문에 공고하고, 인터넷 홈페이지 등을 통하여 공시하여야 합니다.
1. 등록이 취소되거나 말소된 경우
2. 해산 또는 선불전자지급수단 발행 및 관리 업무를 폐지한 경우
3. 파산선고를 받은 경우
4. 선불전자지급수단 발행 및 관리 업무의 정지명령을 받은 경우
5. 제1호부터 제4호까지에 준하는 사유가 발생한 경우
28 조의 2 조 (선불충전금의 신탁 또는 지급보증보험)
① "회사"는 “회원”이 유상으로 충전한 “선불전자지급수단”의 선불충전금 전부를 은행 등 신탁회사에게 신탁하며, 신탁업자에게 안전자산으로 운용하도록 지시합니다. 다만, “회사”는 전월말 기준 전체 선불충전금의 1/10에 해당하는 금액(이하 ‘지급준비금’)까지 보통예금 등 안전자산 중 수시입출이 가능한 형태로 신탁회사에 예치할 수 있습니다.
② “회사”는 신탁된 선불충전금(지급준비금 제외)의 수익자를 “회원”으로 지정하여야 합니다. 다만, 개별 “회원”을 수익자로 지정하기 곤란한 경우 회사와 이해관계가 없는 특수목적법인(SPC)를 수익자로 지정할 수 있습니다.
③ “회사”는 당일 “회원” 및 자금의 변동 내역을 신속하게 익일까지 신탁합니다.
④ 제1항에도 불구하고 불가피한 사유로 선불충전금 중 일부를 신탁하지 아니하고 직접 운용(지급준비금 제외)하는 경우 “회사”는 운용대상 금액 전부에 대해 지급보증보험에 가입하여야 하며, 이 경우에도 “회사”는 운용대상 선불충전금을 안전자산으로 운용합니다.
제 28 조의 3 (착오송금에 관한 사항)
...
② 회사는 수취인에 대한 연락 사실, 수취인의 반환의사 유무, 수취인이 반환의사가 없는 경우 그 사유 등 고객 요청사항에 대한 처리결과 또는 관련 처리 진행상황을 고객이 전항의 ‘착오송금’ 발생사실을 회사에 통지한 날로부터 15일 이내에 고객에게 알려야 합니다.
③ 회사 또는 수취 금융회사를 통한 착오송금 반환 요청에도 수취인이 반환하지 않는 경우, 고객은 ｢예금자보호법｣ 제5장(착오송금 반환지원)에 따라 예금보험공사에 착오송금 반환지원 제도 이용을 신청할 수 있습니다. 단, 연락처를 통한 송금, SNS회원간 송금 거래 등 예금보험공사가 수취인의 실지명의를 취득할 수 없는 거래는 반환지원 신청이 제한됩니다.
...
제 28 조의 4 (기타)
"회사"는 신유형 상품권 표준약관 제4조의 표시사항을 VAR 서비스페이지, 고객센터 등에 표시합니다.
...
2. "회원"이라 함은 본 약관에 동의하고 판매자로부터 "재화 등"을 구매하고 "직불전자지급수단"을 결제수단으로 하여 그 대가를 지급하는 자를 말합니다.
3. "판매자"라 함은 "직불전자지급수단"에 의한 거래에 있어서 "회원"에게 "재화 등"을 판매하는 자를 말합니다.
제 30 조 ("접근매체"의 관리)
① 제 17조는 본 장에 준용합니다.
제 31 조 (직불 결제)
① "회사"는 "회원"이 "판매자"에게 재화 등을 구매 시 직불전자지급수단으로 결제할 경우, 이용 대금을 "회원"의 결제계좌에서 즉시 출금하여 결제하고 출금 사실을 "회원"의 결제계좌에 기록합니다.
② "회원"이 지정계좌를 변경하고자 하는 경우 "회사"가 정한 방법으로 변경할 수 있습니다.
제 32 조 (거래지시의 철회)
① "회원"이 "직불전자지급수단"을 이용하여 자금을 지급하는 경우, "회원"은 "거래지시"된 금액의 정보가 수취인의 계좌가 개설되어 있는 금융회사 또는 전자금융업자의 "전자적 장치"에 입력이 끝나기 전까지 "거래지시"를 철회할 수 있습니다.
...
제 34 조 (정의)
본 장에서 사용하는 용어의 정의는 다음과 같습니다.
1. "전자고지결제서비스"라 함은, 수취인을 대행하여 "회원"이 수취인에게 지급하여야 할 자금의 내역을 "청구서"등의 전자적인 방법으로 고지하고, 자금을 직접 수수하여 그 정산을 대행하는 업무를 제공하는 시스템 및 서비스 일체를 말합니다.
제 35 조 ("접근매체"의 관리)
① 제 17조는 본 장에 준용합니다.
제 36 조 ("거래지시"의 철회)
① "회원"이 "전자고지결제서비스"를 이용한 경우, "회원"은 "거래지시"된 금액의 정보에 대하여 수취인의 계좌가 개설되어 있는 금융회사 또는 "회사"의 계좌의 원장에 입금기록이 끝나거나 "전자적 장치"에 입력이 끝나기 전까지 "거래지시"를 철회할 수 있습니다.
② "회사"는 "회원"의 "거래지시"의 철회에 따라 지급거래가 이루어지지 않은 경우 수령한 자금을 "회원"에게 반환하여야 합니다.
...
1. 전자금융거래서비스 연동 기능: "회사"의 "전자금융거래서비스"를 통해 "제휴 서비스"를 편리하게 이용할 수 있는 기능
2. 혜택 제공 기능: "회원"이 "제휴 서비스"를 사용하면서 "회사"가 제시하는 조건을 충족하는 경우 "회사"가 "회원"에게 선불전자지급수단 적립 등 혜택을 제공하는 기능
3. 기타 "제휴 금융회사"와의 제휴에 따라 제공하는 기능
② "회원"의 개별 "제휴 서비스"의 이용에 대해서는 "회사" 및 "제휴 금융회사"가 해당 서비스에 관하여 적용될 사항을 정한 약관(이하 "개별 약관"이라고 합니다), 운영정책, 이용정책이 본 약관에 우선하여 적용됩니다.
③ "회사"는 본 약관 및 "개별 약관"에 따라 계속적, 안정적으로 "제휴 서비스"를 제공할 의무가 있습니다. 다만, "제휴 금융회사"와의 계약 종료 등과 같은 "회사"의 제반 사정 또는 법률상의 장애 등으로 유지할 수 없는 경우, "회원"에게 사전에 이를 공지한 후 "제휴 서비스"의 전부 또는 일부를 제한·변경하거나 중지할 수 있습니다.

부칙
이 약관은 2024년 4월 22일부터 적용됩니다.
VAR 서비스와 관련하여 궁금하신 사항이 있으시면 VAR 대표번호(000-0000-0000)로 문의 주시기 바랍니다.
`}</pre>
            </div>
          </div>
          <div className={styles.rightCheck}>
            ...
            />
            <label htmlFor="termsCheckbox" className={styles.TermCheckBoxText}>
              (필수) 위 내용을 모두 읽고 전자금융거래 약관에 동의합니다.
            </label>
            <div className={styles.TermCheckBoxText1}>*</div>
          </div>
        </div>
      </div>
      <div className={styles.subscribeInfo2}>
        <div className={styles.subTitle}>포인트 출금하기</div>
        <div>
          <div className={styles.subInfoWrapper2}>
            <div className={styles.subInfoContainer3}>
              <div className={styles.subInfoTitle}>계좌 소유자</div>
              <input
                type="text"
                id="accountOwner"
                name="accountOwner"
                placeholder="계좌 소유자 명 입력"
                value={accountOwner}
          ...
          <div className={styles.subInfoWrapper3}>
            <div className={styles.subInfoContainer3}>
              <div className={styles.subInfoTitle}>입금받을 계좌</div>
              <select
                value={selectedBank}
          ...
          <div className={styles.subInfoWrapper3}>
            <div className={styles.subInfoContainer2}>
              <div className={styles.subInfoTitle}>충전할 금액</div>
              <input
                type="text"
                id="verification"
                name="verification"
                placeholder="만원단위로 출금이 가능합니다."
                value={chargeAmount}
                onChange={handleInputChange}
                required
              />
              <div className={styles.subInfoTitle2}>P</div>
            </div>
      ...
      <button
        className={
          isButtonActive ? styles.chargeButtonActive : styles.chargeButton
        }
